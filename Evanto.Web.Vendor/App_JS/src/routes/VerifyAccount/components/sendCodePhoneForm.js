...
const SendCodePhoneForm = props => {
    const { register, handleSubmit, pristine, reset, submitting, cancel, currentDate, requestPendingLogin } = props
    return (
        <form onSubmit={handleSubmit}>
            <Field name='phone' type='text' labelClass='tbx-send-code-address disabled' component={renderTextField} label={props['##phone']} />
            <button className='submit btn-send-code' type='submit' name='register'>{props['##sendCode']}</button>
        </form>
    )
